...
//		System.out.println(courseSvc.getAll());
		...
		for (int i = 0; i < list.size(); i++) {
		    UserVO user = list.get(i).getUserVO();
		    UserHeadshotOnlyDTO headshotDTO = userSvc.getUserHeadshotDTOById(user.getuId());
		    String attributeKey = "base64Encoded" + i; // 為每個圖片創建一個唯一的屬性鍵
		    ...
		    if (headshotDTO != null && headshotDTO.getuHeadshot() != null) {
		        ...
		        userHeadshots.put(attributeKey, base64Encoded);
		    } else {
	    ...
	    model.addAttribute("userHeadshots", userHeadshots);
